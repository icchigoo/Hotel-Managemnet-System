...
public class UpdateRoom3 extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b1){
            ...
            conn c = new conn();
            try{
            ResultSet rs = c.s.executeQuery("select * from customer3 where number = '"+s1+"'"); 
                       while (rs.next()){
                t1.setText(rs.getString("room"));
               room = rs.getString("room");
                              
              }   
            ResultSet rs2 = c.s.executeQuery("select * from room2 where room = '"+room+"'");  
                      while(rs2.next()){
        ...
        try{
            ...
            String str ="update room2 set available ='"+available+"',status ='"+status+"' where room='"+room+"'";
